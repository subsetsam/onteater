(ns onteater.ui.brand
  "The Onteater brand mark as an inline SVG string. Embedded as a string (rather
   than an external asset) so it survives inlining into the shipped single-file
   build. Shared by the menu-bar mark (shell) and the ontology empty state, so the
   SVG — and its gradient ids — live in exactly one place. Source:
   onteater_logo_alt.svg (a front-facing tamandua head: cream face with big
   rounded ears, two glossy eyes, and a long dark snout).")

(def logo-svg
  "Inline SVG for the Onteater mark. `aria-hidden` since it is decorative wherever
   it is used (a text label always sits beside it)."
  (str
